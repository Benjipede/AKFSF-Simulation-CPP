...
void KalmanFilter::predictionStep(GyroMeasurement gyro, double dt)
{
    if (isInitialised())
    {
        ...
        state = state + dt * time_coefficient;
        state[2] = wrapAngle(state[2]);

        Matrix4d F = Matrix4d::Identity();
        F(0, 2) = -dt * time_coefficient[1];
        F(1, 2) = dt * time_coefficient[0];
        F(0, 3) = dt * cos(gyro.psi_dot);
        F(1, 3) = dt * sin(gyro.psi_dot);
