...
void KalmanFilter::predictionStep(double dt)
{
    ...
    if (isInitialised())
    {
        ...
        // ----------------------------------------------------------------------- //
        // ENTER YOUR CODE HERE
        MatrixXd process = Matrix4d::Identity();
        process(0, 2) = dt;
        process(1, 3) = dt;
        
        MatrixXd LQL = Matrix4d::Zero();
        double var = ACCEL_STD * ACCEL_STD;
        double pos = 0.5 * dt * dt;
        double vel = dt;

        double tl = pos * pos * var;
        double tr = pos * vel * var;
        double lr = vel * vel * var;
        LQL <<     tl, 0, tr, 0,
                   0, tl, 0, tr,
                   tr, 0, lr, 0,
                   0, tr, 0, lr;
        ...
        state = process * state;
        cov = process * cov * process.transpose() + LQL;
...
void KalmanFilter::handleGPSMeasurement(GPSMeasurement meas)
{
    ...
    if(isInitialised())
    {
        ...
        auto y = Vector2d(meas.x, meas.y) - H * state;
